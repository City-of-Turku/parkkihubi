...
def test_archived_parking_data():
    ...
    assert archived_parking.registration_number == ''  # Archiving anonymizes registration numbers
    ...
    assert archived_parking.normalized_reg_num == ''  # Archiving anonymizes normalized registration numbers
...
@pytest.mark.django_db
def test_archive_parkings_mgmt_cmd():
    keep_days = 185
    # Create 20 parkings where:
    #  - first ten are older than 150 days but newer than 185 days
    #  - second ten are older than 185 days
    for age_in_days in [150, 200]:
    ...
    call_command(archive_parkings.Command(), "--keep-days", keep_days)

    assert ArchivedParking.objects.all().count() == 10
    assert Parking.objects.count() == pre_parking_count - 10
...
def test_archive_parkings_mgmt_cmd_dry_run(dry_run_enabled, expected_line):
    end_time = timezone.now() - datetime.timedelta(days=200)
    ...
    (result, stdout, stderr) = call_mgmt_cmd_with_output(
        archive_parkings.Command, '--keep-days', '185', '-v0', *dry_run_opts
    )
